...
FIRMWARE_URL = "https://raw.githubusercontent.com/scuffster/pyScripts/refs/heads/master/gate_code.py"  # Replace with your hosted URL

# Connect to Wi-Fi
def connect_wifi():
    print("Connecting to Wi-Fi...")
    publish_log("Connecting to Wi-Fi...")
    wifi.radio.connect(WIFI_SSID, WIFI_PASSWORD)
    print(f"Connected to {WIFI_SSID}!")
    publish_log(f"Connected to {WIFI_SSID}!")
...
def check_and_update_firmware():
    ...
    publish_log("Checking for updates...")
    try:
        pool = socketpool.SocketPool(wifi.radio)
        ssl_context = ssl.create_default_context()
...
try:
    ...
    publish_log(" Check for OTA updates")
    ...
    # Health Check Timer: Record the last health check time
    ...
    while True:
        try:
            ...
            if time.monotonic() - last_health_check >= 1200:  # 3600 seconds = 1 hour
                publish_log("Health Check: System is operational")
                last_health_check = time.monotonic()  # Reset timer
